...
BEGIN
	...
	WHERE tbl_league_publishergame.MasterGameID is not null AND tbl_league_publisher.UserID = P_UserID AND tbl_league_publisher.Year IN (SELECT YEAR FROM tbl_meta_supportedyear WHERE OpenForPlay = 1 AND Finished = 0);
