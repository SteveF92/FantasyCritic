<template>
  <tr :class="{ 'table-warning': gameSlot && !gameSlot.gameMeetsSlotCriteria }" class="minimal-game-row">
    ...
    <td class="score-column">
      <template v-if="scoreColumnMode === 'RealScore'">{{ score(game.criticScore, decimalsToShow) }}</template>
      <template v-if="scoreColumnMode === 'Empty'"></template>
    </td>

    <td class="score-column" :class="{ 'projected-text': showProjectedScore }">
      <template v-if="scoreColumnMode === 'RealScore'">{{ score(game.fantasyPoints, decimalsToShow) }}</template>
      <template v-if="scoreColumnMode === 'ProjectedPoints'">~{{ score(gameSlot.projectedFantasyPoints, decimalsToShow) }}</template>
